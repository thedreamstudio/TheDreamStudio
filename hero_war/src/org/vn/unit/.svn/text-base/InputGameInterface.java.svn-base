			...
			case start_game:
				...
				for (Enemy enemy : listEnemies) {
					if (enemy.playerId != CurrentUserInfo.mPlayerInfo.ID) {
						...
						for (EnemyType e : mCurrentGameInfo.listEnemytype) {
							if (e.armyType == enemy.armyType) {
								enemyType = e;
								break;
							}
						}
					...
					// Reset map direction
					BaseObject.sSystemRegistry.characterManager
							.setIsMapChangeAndPutAllCharacter(true);
					mCurrentGameInfo.isInGame = true;
